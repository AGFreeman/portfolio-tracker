...
def render_asset_classes():
    st.subheader(
        "Классы активов",
        help=(
            "Задавайте только подклассы (в % всего портфеля). "
            "Доля класса = сумма подклассов и обновляется сама при сохранении."
        ),
    )
    ...
    for ac in classes:
        ...
        with st.expander(
            f"{ac.name} — {sub_sum:.3f}% портфеля "
            f"(сумма подклассов; доля класса не редактируется вручную)"
        ):
            for s in subs:
